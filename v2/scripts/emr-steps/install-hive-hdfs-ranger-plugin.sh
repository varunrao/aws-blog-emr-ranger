...
# Turning this off as it was producing a lot of Audits
sudo sed -i "s|XAAUDIT.SOLR.ENABLE=.*|XAAUDIT.SOLR.ENABLE=false|g" install.properties
...
sudo ln -s /etc/hive/conf $installpath/hive/conf || true
sudo ln -s /usr/lib/hive $installpath/hive/lib || true
